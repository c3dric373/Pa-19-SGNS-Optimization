...
To tune the batch size and create batches of better quality we chose to analyze the distribution of word frequencies. A box plot of the distribution, on the text8 dataset, can be found in Figure \ref{fig:boxplot_freq}. 
\begin{figure}[h]
\centering
...
\end{figure} 
This shows that very few words are responsible for the majority of the context pairs. For example the word \textit{the} creates 50\% of the pairs in the text8 dataset. Therefore if we create arbitrary batches, most words  will appear multiple times in a batch and the learning will be hindered. Therefore we propose a solution where each word will only appear once in a batch. 
\subsubsection{Batches}
To solve this problem we allowed each batch to have a different size. Each batch will contain the most possible pairs where each center word does not appear twice. The problem with this approach is that the batch size decreases very rapidly. This is due to the distribution of words. In consequence the average batch size is small and training time is increased, to remedy this problem we tried to reduce the number of batches, while maintaining the quality of learning. 
\subsubsection{Deletion of outliers}
The idea is to delete every word which is an outlier, according to the whiskers plot. This means every word which frequency is bigger than the frequency of the word that marks the boundary of the 3/4 percentile +1.5 IQR. This technique drastically reduces the dataset size,  and is for some datasets, too drastic. We therefore experimented with different frequencies, that could be a good treshhold. 